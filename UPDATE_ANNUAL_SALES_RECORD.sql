drop procedure if exists crai.UPDATE_ANNUAL_SALES_RECORD;

create
    definer = tuidiadmin@`%` procedure crai.UPDATE_ANNUAL_SALES_RECORD()
begin

    set @range_days = 30;
    set @total_days = 360;

    drop temporary table if exists crai.t_annual_sales_record;

    create temporary table crai.t_annual_sales_record
    with asr_full as
             (select p.product_code                                        as product_code,
              ...
              from crai.sales_record sr
                       inner join crai.product p on sr.product_id = p.id
 ...
 */
    DELETE old
    FROM crai.annual_sales_record old
             inner join crai.product p on old.product_id = p.id
    WHERE p.product_code not in (SELECT DISTINCT new.product_code
                                 FROM crai.t_annual_sales_record new);
 ...
 */
    insert into crai.annual_sales_record (product_id,
                                             sold_day,
                                             annual_amount,
                                             annual_discount,
...
-- Inserire tutti i campi della tabella
    ...
    from crai.t_annual_sales_record new
             inner join crai.product p2 on new.product_code = p2.product_code
