drop procedure if exists crai.UPDATE_SUPPLIER_REGISTRY;

create
    definer = tuidiadmin@`%` procedure crai.UPDATE_SUPPLIER_REGISTRY()
begin
 ...
 */
     ...
     */
    delete old
    from crai.supplier_registry old
             join t_craicommon.supplier_registry new on old.supplier_code = new.supplier_code
    where to_be_deleted = 1;

    -- #########################################################################################
/*Inserisco le nuove istanze dei record esistenti
 */
    insert into crai.supplier_registry (supplier_code, business_name, tax_code, email, address)
-- Inserire tutti i campi della tabella
    select new.supplier_code, new.business_name, new.tax_code, new.email, new.address
    from t_craicommon.supplier_registry new
